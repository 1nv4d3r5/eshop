<?
/**
 * This is a class that can process an image on the fly by either generate a thumbnail,
 * apply an watermark to the image, or resize it.
 *
 * The processed image can either be displayed in a page, saved to a file, or returned to a variable.
 * It requires the PHP with support for GD library extension in either version 1 or 2. If the GD library version 2
 * is available it the class can manipulate the images in true color, thus providing better quality of the results
 * of resized images.
...
class Thumbnail
{
	...
	function size_hard($size_x, $size_y)
	{
		if ($this->img["x"] < $size_x)
		{
			$this->img["x_thumb"] = $this->img["x"];
		}
		else
	...
	function crop()
	{
		...
		$src_aspect  = $this->img["y"] / $this->img["x"];
		$dest_aspect = $this->img["y_thumb"] / $this->img["x_thumb"];

		if ($dest_aspect > $src_aspect)
		{
			$scaleK = $this->img["y_thumb"] / $this->img["y"];
		}
		else
		{
			$scaleK = $this->img["x_thumb"] / $this->img["x"];
		}

		$this->img["x_delta"] = ($this->img["x"] - ($this->img["x_thumb"] / $scaleK));
		$this->img["y_delta"] = ($this->img["y"] - ($this->img["y_thumb"] / $scaleK));

		$this->img["x"] = round($this->img["x"] - $this->img["x_delta"]);
		$this->img["y"] = round($this->img["y"] - $this->img["y_delta"]);

		$this->img["x_delta"] = round($this->img["x_delta"] / 2);
		$this->img["y_delta"] = round($this->img["y_delta"] / 2);
	...
	//Скрипт для изменения цвета изображения
	function rgba_colorize($color)
	{
		$keys  = array(
			'red',
			'green',
		...
		$color = explode(",", $color);
		$color = array_combine($keys, array_values($color));

		imagefilter($this->img['src'], IMG_FILTER_COLORIZE, $color['red'], $color['green'], $color['blue']);
		//imagepng($this->img['src'], $this->img);
	}
	...
	function process()
	{
		...
		if (!isset($this->img["x_delta"]))
		{
			$this->img["x_delta"] = 0;
		}
		if (!isset($this->img["y_delta"]))
		{
			$this->img["y_delta"] = 0;
		}
		...
		if ($gd_version >= 2)
		{
			...
			if ($this->img_watermark != '' && file_exists($this->img_watermark))
			{
				...
				imagecopyresampled(
					$this->img["des"],
					$this->img["watermark"],
					$this->calc_position_H(),
					$this->calc_position_V(),
					0,
					0,
					$this->img["x_watermark"],
					$this->img["y_watermark"],
					$this->img["x_watermark"],
					$this->img["y_watermark"]
				);
			}
			...
			if ($this->txt_watermark != '')
			{
				imagestring(
					$this->img["des"],
					$this->txt_watermark_font,
					$this->calc_text_position_H(),
					$this->calc_text_position_V(),
					$this->txt_watermark,
					$txt_color
				);
			}
